...
public class App extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        testDbButton.setStyle(
                "-fx-background-color: #3b82f6; -fx-text-fill: white; -fx-padding: 10 20; -fx-font-size: 14px; -fx-font-weight: bold; -fx-cursor: hand;");
        testDbButton.setOnAction(e -> testConnection());

        VBox root = new VBox(20);
        root.setAlignment(Pos.CENTER);
        root.setStyle("-fx-background-color: #18181b; -fx-padding: 30;");
    ...
    private void testConnection() {
        ...
        try {
            // Explicitly force loading the MySQL driver to avoid module initialization
            // issues
            ...
            try (Connection ignored = DriverManager.getConnection(jdbcUrl, username, password)) {
                ...
                // If it's an auth error or missing DB, it means the driver is working fine and
                ...
                if (ex.getMessage().contains("Access denied") || ex.getMessage().contains("Unknown database")) {
                    ...
                    statusLabel.setStyle("-fx-text-fill: #f59e0b;"); // Orange
                } else {
